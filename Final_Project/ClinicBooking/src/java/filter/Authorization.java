...
public class Authorization implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
        ...
        if (url.contains("manage") || url.contains("setting") || url.contains("account") || url.contains("dashboard")) {
            if (user != null) {
                if (url.contains("servicemanage") || url.contains("blogmanage")) {
                    if (user.getRole() != null && (user.getRole().getRole_id() == 5 || user.getRole().getRole_id() == 1)) {
                        filterChain.doFilter(servletRequest, servletResponse);
                    } else {
                        response.sendRedirect(request.getContextPath() + ERROR401);
                    }
                } else if (url.contains("patientmanage") || url.contains("doctormanage") || url.contains("appointmentmanage") || url.contains("reservationmanage")) {
