...
public class AccountDAO extends DBContext {
    ...
    PreparedStatement ps = null; // Ném câu lệnh từ netbean sang sql 
    ResultSet rs = null;        // Hiện kết quả trả về 
    DBContext dbc = new DBContext();
    Connection connection = null; // ket noi database
...
//Login
    public Account login(String email, String password) throws SQLException, IOException {
        ...
        try {
            ...
            connection = dbc.getConnection();       // Mở kết nối vs sql 
            ps = connection.prepareStatement(sql); // truyền câu lệnh sql vào 
            ps.setString(1, email);                 // truyền email vào ? thứ nhất 
            ps.setString(2, password);
            rs = ps.executeQuery();                 // Nhận kết quả trả về 
            
            while (rs.next()) { // Lấy dữ liệu từng dòng 
